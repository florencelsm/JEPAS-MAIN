    ...
    for attempt in range(num_retries):
        try:
            ...
            if audio_output_path:
                args += ['-map', '0:1'] + audio_output_args + [audio_output_path]  # 容错加 ?
            ...
            # --- 新增：不可恢复错误（404/Invalid input）直接跳过 ---
            if ('404 Not Found' in stderr) or ('Invalid data found when processing input' in stderr):
                LOGGER.warning(f"Permanent error for {input_path}: {stderr.splitlines()[-1]}. Skipping.")
                return  # 不再重试
            # HTTP 4XX/5XX: 转为 FfmpegInvalidURLError，交由外层逻辑判断备用 URL
...
def download_flickr_video(url, data_dir, ffmpeg_path, ffprobe_path,
    ...
    try:
        ...
        if (not is_retry) and ('404' not in e.error):
            flickr_id = extract_flickr_id(url)
            alt_url = f'https://www.flickr.com/video_download.gne?id={flickr_id}'
            LOGGER.info(f'Retrying with direct link: {alt_url}')
            ...
            LOGGER.warning(f'Skipping URL (permanent error): {url}')
            with open(os.path.join(data_dir, 'failed_urls.txt'), 'a') as fp:
                fp.write(url + '\n')

...
def process_entry(entry, base_output_dir, sanity_check_output_dir, item_index):
    ...
    if not os.path.exists(audio_output_path): # Avoid re-extracting audio
        if not extract_audio_segment(video_output_path, audio_start_time, audio_duration, audio_output_path):
...
def main(json_file_path, output_directory="vgg_ss_extracted_data"):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        futures = [executor.submit(process_entry, entry, output_directory, sanity_check_output_dir, i)
                   for i, entry in enumerate(dataset)]

        # Wait for tasks to complete and retrieve results (or exceptions)
        for future in as_completed(futures):
            try:
                future.result() # This will re-raise any exception caught during the execution of the task
...
if __name__ == "__main__":
    ...
    json_dataset_file = "vggss/vggss.json"

    # You can change the output directory name here
    output_base_directory = "vgg_ss_processed_data"
